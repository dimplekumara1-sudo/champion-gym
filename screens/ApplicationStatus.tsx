...
const ApplicationStatus: React.FC<ApplicationStatusProps> = ({ onBack, onHome }) => {
  ...
  useEffect(() => {
    ...
    const initializeStatus = async () => {
      try {
        ...
        if (!session || !isMounted || isRedirecting) return;

        const { data, error } = await supabase
          .from('profiles')
          .select('approval_status, onboarding_completed, plan')
          .eq('id', session.user.id)
          .single();

        if (error) {
          console.error('[ApplicationStatus] Error fetching approval status:', error);
          // If we get a session error, try to recover
          if (error.message.includes('session') || error.status === 401) {
            console.log('[ApplicationStatus] Attempting session recovery');
        ...
        console.log('[ApplicationStatus] User status:', userStatus, 'Onboarding completed:', data?.onboarding_completed);

        // CRITICAL: Check if user is approved IMMEDIATELY upon component mount
        // Don't wait for state updates or polling
        if (userStatus === 'approved' && data?.onboarding_completed) {
          if (!isRedirecting && isMounted) {
            isRedirecting = true;
            console.log('[ApplicationStatus] ✅ User approved and onboarding completed - AUTO-REDIRECTING TO DASHBOARD');
            setHasRedirected(true);
            // Use a small timeout to ensure state updates happen
            setTimeout(() => {
              onHome();
            }, 0);
            return;
        ...
        console.error('[ApplicationStatus] Error in initializeStatus:', error);
        if (isMounted) {
          setLoading(false);
        }
      }
  ...
  // Safety: If status updates to approved, redirect immediately
  useEffect(() => {
    if (status === 'approved' && !hasRedirected) {
      console.log('[ApplicationStatus] Status effect: Detected approved status, redirecting');
      setHasRedirected(true);
      onHome();
    }
  }, [status, onHome, hasRedirected]);

  const fetchStatus = async () => {
    try {
      // Validate and recover session
      const session = await validateAndRecoverSession();
      if (!session) {
      ...
      if (error) {
        if (error.message.includes('session') || error.status === 401) {
          await validateAndRecoverSession();
          alert('Session recovered. Please try again.');
        } else {
          throw error;
        }
        return;
      ...
      const userStatus = data?.approval_status || 'pending';
      console.log('[ApplicationStatus] Refresh clicked - Status:', userStatus);
      setStatus(userStatus);

      // CRITICAL: Redirect immediately if user is approved
      if (userStatus === 'approved' && data?.onboarding_completed) {
        console.log('[ApplicationStatus] ✅ Status updated - User approved, redirecting now');
        setHasRedirected(true);
        // Direct redirect
        onHome();
        return;
      ...
      console.error('[ApplicationStatus] Error in fetchStatus:', error);
  ...
  return (
    <>
      {/* Auto-redirect if approved - don't show UI */}
      {hasRedirected && (
