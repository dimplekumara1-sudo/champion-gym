...
async function handleUserSync(line: string) {
  ...
  if (esslId && name) {
    // Only update if name is different to avoid triggering profile updates unnecessarily
    const { data: existing } = await supabase
      .from("profiles")
      .select("username")
...
async function handleAttendance(line: string, sn: string) {
  ...
  if (esslUserId && logTime) {
    ...
    const { data: profile } = await supabase
      .from("profiles")
      .select("id, essl_blocked, plan_expiry_date, grace_period")
      .eq("essl_id", esslUserId.toString())
      .maybeSingle();

    // BLOCKING VALIDATION: Reject attendance from blocked users
    if (profile && profile.essl_blocked) {
      console.log(`[BLOCKED] Access rejected for ${esslUserId} - marked as essl_blocked=true`);
      return;
    }

    // EXPIRY VALIDATION: Reject attendance from expired users (+ grace period)
    if (profile && profile.plan_expiry_date) {
      const now = new Date();
      const expiryDate = new Date(profile.plan_expiry_date);

      // Get global grace period from app_settings if user doesn't have custom grace_period
      let graceDays = profile.grace_period ?? 0;
      if (graceDays === null || graceDays === undefined) {
        const { data: appSettings } = await supabase
          ...
          .eq("id", "gym_settings")
          .maybeSingle();
        graceDays = appSettings?.value?.global_grace_period || 0;
      }

      const finalExpiryDate = new Date(expiryDate);
      finalExpiryDate.setDate(finalExpiryDate.getDate() + graceDays);
      finalExpiryDate.setHours(23, 59, 59, 999);

      if (now > finalExpiryDate) {
        console.log(`[EXPIRED] Access rejected for ${esslUserId} - plan expired with grace period`);
        return;
      }
    }
